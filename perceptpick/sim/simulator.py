...
class SimulatorBase:
    ...
    def _reset(self, plane_and_gravity=False):
        ...
        if self.verbose:
            ...
            # Configure debug visualizer to disable keyboard/mouse controls except pan and zoom
            self._p.configureDebugVisualizer(p.COV_ENABLE_KEYBOARD_SHORTCUTS, 0)
            self._p.configureDebugVisualizer(p.COV_ENABLE_MOUSE_PICKING, 0)
            self._p.configureDebugVisualizer(p.COV_ENABLE_VR_PICKING, 0)
            self._p.configureDebugVisualizer(p.COV_ENABLE_VR_TELEPORTING, 0)
            self._p.configureDebugVisualizer(p.COV_ENABLE_VR_RENDER_CONTROLLERS, 0)
            self._p.configureDebugVisualizer(p.COV_ENABLE_RGB_BUFFER_PREVIEW, 0)
            self._p.configureDebugVisualizer(p.COV_ENABLE_DEPTH_BUFFER_PREVIEW, 0)
            self._p.configureDebugVisualizer(p.COV_ENABLE_SEGMENTATION_MARK_PREVIEW, 0)
            self._p.configureDebugVisualizer(p.COV_ENABLE_GUI, 1)
            # Keep pan and zoom enabled by not disabling them
    ...
    def draw_camera_coordinate_frame(self, camera_pose, axis_length=0.1):
        """
        Draw the camera coordinate system as colored lines.
        
        :param camera_pose: 4x4 transformation matrix of the camera
        :param axis_length: length of the coordinate axes in meters
        """
        ...
        # Camera coordinate system axes (in camera frame)
        # X axis (red) - right
        x_end = pos + rot_matrix[:, 0] * axis_length
        self._p.addUserDebugLine(pos, x_end, [1, 0, 0], lineWidth=3)
        
        # Y axis (green) - down  
        y_end = pos + rot_matrix[:, 1] * axis_length
        self._p.addUserDebugLine(pos, y_end, [0, 1, 0], lineWidth=3)
        
        # Z axis (blue) - forward
        z_end = pos + rot_matrix[:, 2] * axis_length
        self._p.addUserDebugLine(pos, z_end, [0, 0, 1], lineWidth=3)
        
        # Add labels
        self._p.addUserDebugText("Camera", pos + [0, 0, 0.02], [1, 1, 1], textSize=1.5)
        self._p.addUserDebugText("X", x_end, [1, 0, 0], textSize=1.2)
        self._p.addUserDebugText("Y", y_end, [0, 1, 0], textSize=1.2)
        self._p.addUserDebugText("Z", z_end, [0, 0, 1], textSize=1.2)

    def add_object(self, object_instance, fixed_base=False, no_collision=False):
        """
        ...
        :param no_collision: if True, the object will not collide with anything
        ...
        if no_collision:
            # Disable all collision detection for this object
            self._p.setCollisionFilterGroupMask(object_id, -1, 0, 0)
            # Also remove collision shapes entirely
            num_joints = self._p.getNumJoints(object_id)
            # Disable collision for base
            self._p.changeDynamics(object_id, -1, collisionMargin=0, activationState=0)
            # Disable collision for all links
            for joint_idx in range(num_joints):
                self._p.changeDynamics(object_id, joint_idx, collisionMargin=0, activationState=0)

        if self.verbose:
            # Use custom color if available, otherwise use automatic color
            if hasattr(object_instance, '_color') and object_instance._color is not None:
                color = object_instance._color
                if len(color) == 3:  # RGB, add alpha
                    color = list(color) + [1.0]
                self._p.changeVisualShape(object_id, -1, rgbaColor=color)
            else:
    ...
    def add_scene(self, scene, with_bg_objects=True):
        ...
        if with_bg_objects:
            for bg_instance in scene.bg_objects:
                self.add_object(bg_instance, fixed_base=True, no_collision=True)
...
class GraspSimulator(SimulatorBase):
    ...
    """
    def __init__(self, scene, verbose=False, plane_and_gravity=False, camera_pose=None):
    ...
    def _wait_for_user(self):
        if self.verbose:
            print('Scene loaded with GT (green) and EST (red) meshes. Starting simulation in 0.5 seconds...')
            time.sleep(0.5)
    ...
    def _reset_scene(self):
        ...
        # Draw camera coordinate frame if available
        if self.verbose and self.camera_pose is not None:
            self.draw_camera_coordinate_frame(self.camera_pose)
    ...
    def check_collisions(self, gripper, target):
        ...
        Skip ground collision check since we don't use ground plane.
        ...
        """
        # Skip checking collisions against environment objects (no ground plane)
        _log.debug('skipping ground collision check - no ground plane used')
    ...
    def execute_grasp(self, gripper_type, grasp, target, gripper_scale=1.0, gripper_opening_width=1.0):
        ...
        :return: GraspScore
        """
        print(f"  Step 1: Resetting scene and loading fresh gripper...")
        
        # First reset the scene to clean state
        self._reset_scene()
        ...
        # Allow settling without gravity
        self.step(seconds=0.1)

        print(f"  Step 2: Checking initial collisions...")
        result = self.check_collisions(gripper, target)
        if result != GraspScores.SUCCESS:
            print(f"  Failed at collision check: {GraspScores.score2description(result)}")
            self._reset_scene()
            return result

        print(f"  Step 3: Closing gripper...")
        _log.debug(f'gripper joint states before closing: {gripper.joint_positions}')
        _log.debug('closing gripper...')
        gripper.close()
        _log.debug('GRIPPER CLOSED')
        _log.debug(f'gripper joint states after closing: {gripper.joint_positions}')
        
        # Allow settling after closing gripper
        self.step(seconds=0.2)

        print(f"  Step 4: Enabling gravity after gripper close...")
        # Enable gravity after gripper closes
        self._p.setGravity(0, 0, -9.81)
        
        # Allow some settling time with gravity
        self.step(seconds=0.3)

        print(f"  Step 5: Checking if contact is established...")
        _log.debug('checking contacts...')
        if not self._contact_established(gripper, target):
            print(f"  Failed: No contact with target object established")
            _log.debug('no contact with target object established')
            self._reset_scene()
            return GraspScores.NO_CONTACT_ESTABLISHED
        print(f"  Contact established successfully")
        _log.debug('CONTACT ESTABLISHED')

        print(f"  Step 6: Lifting object...")
        # start lifting
        ...
        try:
            ...
            if not arrived:
                print(f"  Failed: Did not arrive at desired target position")
                _log.warning('did not arrive at desired target position')
                self._reset_scene()
                return GraspScores.SLIPPED_DURING_LIFTING
        except Exception as e:
            print(f"  Failed: Error during lifting: {e}")
            _log.warning(f'Error during lifting: {e}')
            self._reset_scene()
            return GraspScores.SLIPPED_DURING_LIFTING
            
        _log.debug('DONE LIFTING')

        print(f"  Step 7: Final contact check...")
        # check again if object is still in contact
        _log.debug('checking contacts...')
        if not self._contact_established(gripper, target):
            print(f"  Failed: Object slipped during lifting")
            _log.debug('no contact with target object established')
            self._reset_scene()
            return GraspScores.SLIPPED_DURING_LIFTING
    ...
    @classmethod
    def simulate_single_grasp(cls, grasp, scene, target_object, gripper_type, plane_and_gravity=False):
    ...
    @classmethod
    def simulate_graspset(cls, graspset, scene, target_object, gripper_type, plane_and_gravity=False, max_workers=None):
